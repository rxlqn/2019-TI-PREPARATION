...
void ADF4110_GPIO_init(void)
{
    __HAL_RCC_GPIOA_CLK_ENABLE();

    GPIO_Initure.Pin = TriWire_IO | TriWire_SCLK | TriWire_LE | TriWire_CE;
    GPIO_Initure.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_Initure.Pull = GPIO_PULLUP;
    GPIO_Initure.Speed = GPIO_SPEED_FAST;
    HAL_GPIO_Init(TriWire_Port, &GPIO_Initure);

    HAL_GPIO_WritePin(TriWire_Port, TriWire_IO | TriWire_SCLK | TriWire_LE | TriWire_CE, GPIO_PIN_SET);
}
...
uint8 HalSynInit (void)
{
    ...
    halSynSetFunc(ADF4113_SET_CURRENT       , ADF4113_ICP0              );
...
void TriWire_WriteByte(uint8 output_data)
{
    ...
    {
        SCLK_L;								// 时钟拉低
        if(output_data & 0x80)				// 先发高位
            IO_H;							// 1
        else
            IO_L;							// 0
        delay_time(TriWireDELAY);			// min 25ns
        SCLK_H;								// 时钟拉高
        delay_time(TriWireDELAY);
