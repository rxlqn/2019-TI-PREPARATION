...
{  						
   ...
   if(mode)
   {
        for(;t<(t2+1+size*size/4);t++)
        {
           switch(size)
           {
               case 16  :temp = asc16[t];break;
               case 24  :temp = asc24[t];break;
               case 32  :temp = asc32[t];break;
               case 48  :temp = asc48[t];break;
               case 64  :temp = asc64[t];break;
   //            case 96  :temp = asc96[t];break;
        //       case 128 :temp = asc128[t];break
           }
           for(t1=0;t1<8;t1++)
           {
