...
//  arm_fir_init_f32(&LPF_instance, NUMTAPS, (float32_t *)FIRCoeff_f32, firStateF32, BLOCKSIZE);
	...
	for( int i = 0; i < 320; i++ )   
	{
			 float temp=(float)(2*3.141592653)*5000/(float)1000000*(float)i;
			 noise[i] = arm_sin_f32(temp);	
	}
	// 生成含噪信号
	for( int i = 0; i < 320; i++ )   
	{
			 float temp1=(float)(2*3.141592653)*5000/(float)1000000*(float)i;
			 input[i] = arm_sin_f32(temp1);	
			 float temp2=(float)(2*3.141592653)*10000/(float)1000000*(float)i;
			 input[i] = arm_sin_f32(temp2) + input[i];	
	}
